...
class BiEncoderModule(torch.nn.Module):
    def __init__(self, params):
        ...
        self.config = ctxt_bert.config  # type: ignore
...
class BiEncoderRanker(torch.nn.Module):
    ...
    def save_model(self, output_dir):
        ...
        output_model_file = os.path.join(output_dir, WEIGHTS_NAME)  # type: ignore
        output_config_file = os.path.join(output_dir, CONFIG_NAME)  # type: ignore
        torch.save(model_to_save.state_dict(), output_model_file)  # type: ignore
        model_to_save.config.to_json_file(output_config_file)  # type: ignore
    ...
    def forward(
        ...
        if mst_data is not None:
            ...
            )  # batchsize x knn(dict) x embed_size
        ...
        if label_input is None:
            target = torch.LongTensor(torch.arange(bs))  # type: ignore
            ...
            if pos_neg_loss:
                loss = torch.mean(
                    torch.sum(
                        ...
                        - torch.log(1 - torch.softmax(scores, dim=1) + 1e-8)  # type: ignore
                        * (1 - label_input),
                        dim=1,
                    )
                )
