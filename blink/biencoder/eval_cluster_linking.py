...
def get_query_nn(
    ...
    nn_idxs = nn_idxs.astype(np.int64).flatten()  # type: ignore
...
def main(params):
    ...
    test_dict_vecs = torch.tensor(
        list(map(lambda x: x["ids"], test_dictionary)),  # type: ignore
        dtype=torch.long,
    )
    # Store test mention token ids
    test_men_vecs = test_tensor_data[:][0]  # type: ignore
    ...
    if params["transductive"]:
        if os.path.isfile(train_tensor_data_pkl_path) and os.path.isfile(  # type: ignore
            train_mention_data_pkl_path  # type: ignore
        ):
            print("Loading stored processed train data...")
            with open(train_tensor_data_pkl_path, "rb") as read_handle:  # type: ignore
                train_tensor_data = pickle.load(read_handle)
            with open(train_mention_data_pkl_path, "rb") as read_handle:  # type: ignore
            ...
            logger.info("Read %d test samples." % len(test_samples))  # type: ignore
            ...
            with open(train_tensor_data_pkl_path, "wb") as write_handle:  # type: ignore
                pickle.dump(
                    train_tensor_data, write_handle, protocol=pickle.HIGHEST_PROTOCOL
                )
            with open(train_mention_data_pkl_path, "wb") as write_handle:  # type: ignore
                pickle.dump(
                    train_mention_data, write_handle, protocol=pickle.HIGHEST_PROTOCOL
                )

        # Store train mention token ids
        train_men_vecs = train_tensor_data[:][0]  # type: ignore
        ...
        if use_types:
            if embed_data is not None:
                ...
                if "dict_idxs_by_type" in embed_data:
                    ...
                    dict_idxs_by_type = data_process.get_idxs_by_type(test_dictionary)  # type: ignore
                ...
                dict_embeds, dict_indexes, dict_idxs_by_type = (  # type: ignore
                    data_process.embed_and_index(
                        ...
                        n_gpu=n_gpu,
                        corpus=test_dictionary,  # type: ignore
                        force_exact_search=params["force_exact_search"],
