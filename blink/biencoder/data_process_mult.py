...
def process_mention_data(
    ...
    if logger:
        ...
        for sample in processed_samples[:5]:
            ...
            for label in sample["label_idxs"]:
                if label == -1:
                    break
                logger.info(
                    f"Label {label} tokens : "
                    + " ".join(entity_dictionary[label]["tokens"])
                )
                logger.info(
                    f"Label {label} ids : "
                    + " ".join([str(v) for v in entity_dictionary[label]["ids"]])
                )
...
def build_index(embeds, force_exact_search, probe_mult_factor=1):
    ...
    if (
        ...
        index.train(embeds)  # type: ignore
        index.add(embeds)  # type: ignore
        index.nprobe = nprobe
    return index
    ...
    with torch.no_grad():
        ...
        for ent_type in corpus_idxs:
            search_indexes[ent_type] = build_index(
                embeds[corpus_idxs[ent_type]],  # type: ignore
                force_exact_search,
